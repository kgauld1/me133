...
class Kinematics:
    ...
    def grav(self, theta):
        ...
        # As we are walking up, also store the position and joint
        # axis, ONLY FOR ACTIVE/MOVING/"REAL" joints.  We simply put
        # each in a list, and keep an index counter.
        plist = []
        elist = []
        index = 0
        ...
        for (joint, link) in zip(self.joints, self.links):
            ...
            if (link.inertial is not None):
                for k in range(0, index):
                    ...
                    fg = link.inertial.mass * np.array([0, 0, -9.8])
                    # Get the current motor position
                    pci = p_from_T(T @ T_from_URDF_origin(link.inertial.origin))
                    # Add the torque component to the corresponding index of the gravity vector
                    grav[k] += fg @ np.cross(elist[k].T, (pci - plist[k]).T).T
